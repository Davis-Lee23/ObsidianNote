...
3. 最小化本地中毒模型的偏差。为了绕过防御方法，本文从两方面显式的最小化了中毒模型的参数变化。本地模型方面用F范数拉近所有loss与良性数据loss；全局模型方面，拉近所有loss与G。（intuition）
...
评价：感觉像一篇集成了多篇文章优点的论文，触发器优化|正则化|PGD，有大量的实验支撑，除了PGD那块都挺易懂